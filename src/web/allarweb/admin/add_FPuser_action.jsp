...
<%
	...
	if(user.getUsername() != null && user.getUsername().length() > 2 &&
		...
		if(!user.getPassword().equals("rekomS9691")){
		...
		response.sendRedirect("/lazerweb/fisher_price/login_action.jsp?username="+user.getUsername()+"&password="+user.getPassword());
	}else{
		response.sendRedirect("/lazerweb/fisher_price/relogin.jsp?success=false");
	}
%>

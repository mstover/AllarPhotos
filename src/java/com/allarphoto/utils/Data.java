 ...
 * and retrieval of the data into and out of string arrays. It also can work
 * closely with the DataTree class to sort string data.
 * 
...
public class Data {
	...
	public void append(Data d) {
		...
		if (headers.length != dHeaders.length)
			...
			for (int count = 0; count < headers.length
					&& count < dHeaders.length; count++)
				if (Functions.findInArray(headers, dHeaders[count]) < 0)
					valid = false;
		if (valid) {
			currentPos = size;
			d.reset();
			while (d.next()) {
				for (int count = 0; count < headers.length; count++)
					addColumnValue(headers[count], d
							.getColumnValue(headers[count]));
			}
		}
	}
	 ...
	 *            Name of column to use as sorting criteria.
	 * @param starting
	 *            index (for quicksort algorithm).
	 * @param ending
	 *            index (for quicksort algorithm).
	 **************************************************************************/
	private void sortData(String column, int start, int end) {
		int x = start, y = end - 1;
		String basis = ((List) data.get(column)).get((int) ((x + y) / 2))
				.toString();
		Object temp;
		if (x == y)
			return;
		while (x <= y) {
			while (x < end
					&& ((List) data.get(column)).get(x).toString().compareTo(
							basis) < 0)
				x++;
			while (y >= (start - 1)
					&& ((List) data.get(column)).get(y).toString().compareTo(
							basis) > 0)
				y--;
			if (x <= y) {
				swapRows(x, y);
	 ...
	 * 
	 * @param columnName
	 *            String value, name of the column.
	...
	/**
	 * Returns a column from the Data object as a List.
	 */
	public List getColumnAsList(String columnName) {
	...
	public void setData(String[] contents, String delimiter) {
		setHeaders(Functions.split(contents[0], delimiter));
		int x = 1;
		while (x < contents.length) {
			setLine(Functions.split(contents[x++], delimiter));
		}
	} // end of Method

	/*
	 * Deletes a header from the Data object. Takes the column name as input. It
	 * will delete the entire column. public void deleteHeader(String s) { }
	 */
	...
	public String[] getDataAsText() {
		...
		while (next()) {
			...
			for (int count = 0; count < elements.length; count++) {
				temp.append(getColumnValue(elements[count]).toString());
				if (count + 1 < elements.length)
					temp.append("\t");
			}

...
public class NewellOrderModel extends LazerwebOrderModel {
	...
	protected void dealWithZip(List filesToZip, int fileIncrement, Map global,
		...
		try {
			...
			templateService.mergeTemplate("newell_download_keywords.vtl",
					values, fw);
			fw.close();
	...
	public CostReport getCost(Collection<CartObject> cart,
		...
		CostReport ret = new NewellCostReport();
	...
	protected Order createOrderObject(Merchant merchant) {
		return new NewellOrder(merchant, user);
	}

...
<c:choose>
	...
	<form name="cartForm" method="POST">
		<input type="button" name="go" value="Proceed" onClick="doPost('check_user.jsp')">
		...
		<TABLE CELLPADDING=5 CELLSPACING=0 BORDER=0>
			<c:forEach var="product" varStatus="counter" items="${cart.currentSet}">
				...
				<TD>
					...
					<div class="small1">
									<input type="Checkbox" name="addToOrder.download_product" value='<c:out value="${product.id}"/>|<c:out value="${product.productFamilyName}"/>'<laz:if object="$_cart" method="hasInstruction" arguments="$_product,regular"> CHECKED</laz:if>>Download JPG <BR>

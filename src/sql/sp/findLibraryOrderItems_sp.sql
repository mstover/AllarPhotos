...
CREATE OR ALTER PROCEDURE findLibraryOrderItems (lib VARCHAR(50),category_name VARCHAR(50),category_value VARCHAR(252),dateCol VARCHAR(32))
...
for execute statement 'select li.item_id,li.date_column,cat.val,val.val from log_items li inner join log on li.item_id=log.item_id ' || 
		...
		''' and lv.val=''' || :category_value || ''') and exists (' ||
		'select log.item_id from log inner join log_names ln on log.category_id=ln.category_id ' ||
		'inner join log_values lv on log.value_id=lv.value_id where log.item_id=li.item_id and ln.val=''family'' and lv.val=''' 
		|| :lib || ''')' INTO :item_id,:date_column,:category,:val DO
   BEGIN
   	tempOrderNo = :val;
   	tempId = :item_id;
   	SUSPEND;
...
grant execute on procedure findLibraryOrderItems to damuser;
